...
\subsection{Wilson--Majorana 与 two-flavor staggered Gross--Neveu 模型}
\path{Ref/Wilson_Majorana_Fermion_and_Two_Flavor_Staggered_GN.pdf} 采用与式 (2.8) 相同的拆分结构：
\begin{equation}
 Z=\sum_{\{x,t\}}\int\prod_n
 T_{x_n,t_n,x_{n-\hat1},t_{n-\hat2}}\,
 G_{n;x_n,t_n,x_{n-\hat1},t_{n-\hat2}} .
\end{equation}
Wilson--Majorana 模型的数值张量由文献式 (2.9) 给出：
\begin{equation}
 T_{ijkl}=\int \dd\eta_1\dd\eta_2\dd\chi_1\dd\chi_2\,
 e^{(m_\eta+2)\eta_1\eta_2+(m_\chi+2)\chi_1\chi_2}
 (-1)^{i_3+i_4}\,{\cal M}_{ijkl},
\end{equation}
其中 ${\cal M}_{ijkl}$ 是式 (2.9) 打印顺序中的物理场单项式。
实现文件 \path{examples/Wilson_Majorana_Fermion.jl} 对每个占据 bit pattern 直接做 Berezin 积分，并把四个 4-bit 超腿
\((i,j,k,l)\) 物化为 Grassmann 多项式腿；因此对应的系数张量形状为
\((16,16,16,16)\)。

Two-flavor staggered Gross--Neveu 模型的 onsite 相互作用为
\begin{equation}
 e^{U\psi_1\psi_2\psi_3\psi_4}.
\end{equation}
文献式 (5.2) 被实现为
\begin{equation}
 T_{ijkl}=\int\prod_{a=1}^4\dd\psi_a\,
 e^{U\psi_1\psi_2\psi_3\psi_4}
 \prod_{s=1}^4\left(\frac{\eta_{n,1}}2\right)^{x_{n,s}}
 \prod_{s=1}^4\left(\frac{\eta_{n,2}}2\right)^{t_{n,s}}
 {\cal N}_{ijkl},
\end{equation}
其中 ${\cal N}_{ijkl}$ 是式 (5.2) 中
\(\psi_4^{l_4}\psi_3^{l_3}\psi_2^{l_2}\psi_1^{l_1}\cdots
\psi_4^{i_4}\psi_3^{i_3}\psi_2^{i_2}\psi_1^{i_1}\) 的打印顺序。
实现文件 \path{examples/Two_Flavor_Staggered_Gross_Neveu.jl} 逐项生成所有非零系数，并暴露同样的 \((16,16,16,16)\) 系数张量布局。
...
Wilson--Majorana & 两个 Majorana 二分量场；文献把数值张量 $T$ 与 Grassmann 张量 $G$ 拆开，四条腿均为 4-bit 超指标。\\
Two-flavor staggered Gross--Neveu & 四分量 staggered 场与 onsite 四费米项；式 (5.2) 的四个 4-bit 超腿给出 $16^4$ 系数张量。\\
...
\section{Nearest-neighbor compiler layer in code}
The symbolic implementation now separates the universal local Grassmann
integral from the model-dependent decomposition.  A compiled nearest-neighbor
tensor specification contains
\begin{equation}
 \mathcal T = \int D\psi\,\exp[-S_{\rm onsite}(\psi,\bar\psi)]
 \prod_c \exp[B_c(\psi,\bar\psi;\eta_c,\bar\eta_c)],
\end{equation}
together with the external leg order, channel tags, and metadata needed to
compare with a paper convention.  In the Julia source this is represented by
\path{LocalChannel}, \path{NearestNeighborTensorSpec}, and
\path{compile_nearest_neighbor_tensor}.  The higher-level structured interface
\path{NearestNeighborAction} collects onsite terms \path{OnsiteTerm} and
factorized nearest-neighbor hopping terms \path{FactorizedHoppingTerm}, then
lowers them to the same tensor specification by \path{nearest_neighbor_tensor_spec}.
A rank-one hopping factor
\begin{equation}
 \exp\{L(\psi,\bar\psi)R(\psi',\bar\psi')\}
\end{equation}
is entered after Grassmann Hubbard--Stratonovich splitting as two local
channels, for example
\begin{equation}
 B_L = \alpha L\eta,\qquad B_R=\beta\bar\eta R,
\end{equation}
with the signs and square-root normalisations fixed by the reference
convention.  The compiler does not hide this choice: it records the resulting
leg order and the channel tags, because the examples above show that the same
closed partition function may correspond to different published elementary
tensors if auxiliary legs or Berezin measures are reordered.

This layer is therefore ``universal'' in the following precise sense.  Once an
arbitrary nearest-neighbor fermion action has been decomposed into onsite terms
and local channel exponents, the package can derive the elementary Grassmann
tensor by symbolic expansion and Berezin integration.  The remaining
model-specific work is the exact channel decomposition itself: dense hopping
matrices, gauge link sums, source terms, and four-fermion interactions must be
factorized into a finite list of local channels before invoking the compiler.
For the models audited in this document, those decomposition rules are encoded
in the corresponding files under \path{examples/}.
